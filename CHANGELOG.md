...
- Settings > Theme > Background picks the shade of the greys in the dark and light appearances: blue, the new default, the neutral grey from before, purple, green or warm.
...
- Settings > Downloads > Song cache keeps a copy of the songs you stream, up to a size you choose, so they play again without using data or a connection, and the next song in the queue is fetched ahead so it plays from the phone; Cached songs lists them and deletes them one by one or all at once (#180).
...
- The row under the player controls has lyrics, playback speed, sleep timer and an equalizer that does nothing yet, next to devices and the queue, and Settings > Player > Bottom row turns each one on or off and puts them in any order; the sleep timer shows the minutes left.
