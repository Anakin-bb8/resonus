...
- A queue left on another player is picked up on its own, when the app is opened or come back to and nothing is playing here: the server says who wrote its copy last and when, so it is taken only if it is newer than this device's own last word on it. The ⋯ of the queue screen asks for it at any other moment.
